...
class GraphService:
    ...
    async def run(self, payload: Dict[str, Any], ctx=None) -> Dict[str, Any]:
        ...
        try:
            ...
            # 设置全局取消标志，通知内部线程
            cancel_manager.mark_cancelled(run_id)
    ...
    def cancel_run(self, run_id: str, ctx: Optional[Context] = None) -> Dict[str, Any]:
        ...
        同时设置全局取消标志，通知内部线程池立即停止处理。
        """
        logger.info(f"Attempting to cancel run_id: {run_id}")

        # 立即设置全局取消标志，让线程池中的任务能快速检测到
        cancel_manager.mark_cancelled(run_id)
...
async def http_get_progress(run_id: str, request: Request):
    """
    查询工作流执行进度
    ...
    - 批次信息
    """
    ctx = new_context(method="get_progress", headers=request.headers)
    request_context.set(ctx)
    logger.info(f"Received progress query for run_id: {run_id}")

    from utils.progress_manager import progress_manager
    progress = progress_manager.get_progress(run_id)

    if progress is None:
        raise HTTPException(
            status_code=404,
            detail=f"No progress found for run_id: {run_id}. The task may not have started or has already completed and been cleaned up."
        )
